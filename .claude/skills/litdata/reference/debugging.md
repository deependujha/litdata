...
- **Cache grows without bound** → set `max_cache_size` or `MAX_CACHE_SIZE` (default adapts to free disk). Eviction is gated by `.cnt` refcount + `FileLock`. Stale `.lock`/`.cnt` files from a crash can block deletion — `litdata cache clear`. Peak disk ≈ `num_workers × max_pre_download × chunk_size` (async floor often makes `max_pre` 4).
